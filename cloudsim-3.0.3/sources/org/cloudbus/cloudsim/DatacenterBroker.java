...
import org.cloudbus.cloudsim.examples.Dag_Node;
import org.cloudbus.cloudsim.examples.TimeComparator;
...
public class DatacenterBroker extends SimEntity {
	...
	public HashMap<Integer, Dag_Node> cloudlethashmap = new HashMap<>();
	...
	public void setcloudlethashmap(HashMap<Integer, Dag_Node> cloudlethashmap) {
		this.cloudlethashmap = cloudlethashmap;
	}
public void shiftqueues(Queue<Dag_Node> extract_queue,List<PriorityQueue<Dag_Node>> pqlist,List<Integer> submited)
{
	List<Integer>modified=new ArrayList<>(submited);
	while (!extract_queue.isEmpty()) {
		Dag_Node node1 = extract_queue.poll();
		for (Dag_Node child : node1.getChildprocesses()) {
			boolean findflag=true;
			for(Dag_Node parents:child.getParent_process())
			{
				if(!modified.contains(parents.getP_id()))
				{
					findflag=false;
				}
			}
			if(findflag)
			{
				PriorityQueue<Dag_Node> temppque = pqlist.get(child.getInit_qid());
				temppque.remove(child);
				child.setInit_qid(child.getInit_qid() - 1);
				temppque = pqlist.get(child.getInit_qid());
				temppque.add(child);
				modified.add(child.getP_id());
				pqlist.set(child.getInit_qid(), temppque);
			}
		}
	}
}
	...
	protected void submitCloudlets() {
		List<Cloudlet> real_cloudletlist = getCloudletList();
		List<PriorityQueue<Dag_Node>> pqlist = new ArrayList<>();
		List<Integer> submited=new ArrayList<>();
		HashMap<Integer, Cloudlet> cloudletMap = new HashMap<>();
		int qno = 0;
		PriorityQueue<Dag_Node> pque = new PriorityQueue<Dag_Node>(10, new TimeComparator());
		for (Cloudlet cloudlet : real_cloudletlist) {
			Dag_Node node = cloudlethashmap.get(cloudlet.getCloudletId());
			cloudletMap.put(cloudlet.getCloudletId(), cloudlet);
			if (node.getInit_qid() == qno) {
				pque.add(node);
			} else {
				qno = node.getInit_qid();
				pqlist.add(pque);
				pque = new PriorityQueue<Dag_Node>(10, new TimeComparator());
				pque.add(node);
			}
		}
		pqlist.add(pque);;
		...
		while (!pque.isEmpty()) {
			...
			while (vmctr < no_of_vms && !pque.isEmpty()) {
				...
				submited.add(node.getP_id());
				pqueNew.remove(node);
				for (Dag_Node child : node.getChildprocesses()) {
					boolean findflag=true;
					for(Dag_Node parents:child.getParent_process())
					{
						if(!submited.contains(parents.getP_id()))
						{
							findflag=false;
						}
					}
					...
					{
						child.setInit_qid(child.getInit_qid() - 1);
						extract_queue.add(child);
						pqueNew.add(child);
					}
				}
				shiftqueues(extract_queue, pqlist,submited);
				...
				if(node.getParent_process().size()<=0)
					{
						sendNow(getVmsToDatacentersMap().get(vm.getId()), CloudSimTags.CLOUDLET_SUBMIT, cloudlet);
					}
				else {
					double delay=node.getdelay();
			...
			pque = new PriorityQueue<Dag_Node>(pqueNew);
			pqlist.set(0,pque);
		}
